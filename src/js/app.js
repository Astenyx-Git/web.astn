...
class App {
  ...
  flushCurrentEditsToCache() {
    ...
    if (editedData) {
      // Merge: start from original cached object, overlay edited fields
      // This preserves non-editable fields like id, parentId, bookId, order, etc.
      const cachedData = this.reader.assetCache.get(this.selectedAssetId);
      let originalObj = {};
      if (cachedData) {
        try {
          originalObj = this.renderer.parseJson(cachedData);
        } catch (e) { /* ignore parse error, use empty object */ }
      }
      const mergedObj = Object.assign({}, originalObj, editedData);
      const data = new TextEncoder().encode(JSON.stringify(mergedObj));
